...
public class CollectorGame extends GridExploreGame {
	...
	private static final boolean NO_WALLS = false;
	private static boolean SEE_WALLS = !NO_WALLS && true;
	private static int SIZE = 4;
	private static final String SOURCE_NAME = "Collector"+SIZE+"NoWalls";
	private static final String SAVE_NAME = SOURCE_NAME;//"Collector"+SIZE+(NO_WALLS?"NoWalls":"Walls");
	...
	public static CollectorGame trainedGame(int size, int learnIterations, int sampleSizeMultiplier, int m, int repaintMs) {
		...
		ModelLearnerHeavy storedModeler = Utils.loadModelerFromFile(SAVE_NAME, size*size*sampleSizeMultiplier);
		if (storedModeler == null) {
			game.modeler = trainedModeler(size*size, size*size*4, game, sampleSizeMultiplier, repaintMs,
