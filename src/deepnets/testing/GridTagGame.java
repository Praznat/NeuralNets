...
public class GridTagGame extends GridGame {
	...
	static {
		ACTION_CHOICES.add(new double[] {0}); //CLOCKWISE
		ACTION_CHOICES.add(new double[] {1}); //COUNTERCLOCKWISE
	}
...
//		testAbduction(false);
	...
	protected static void testAbduction(boolean clockwise) {
		...
		for (int t = 0; t < turns; t++) {
			...
			game.chosenAction = RandomUtils.randomOf(ACTION_CHOICES);
	...
	protected static void testExplore() {
		...
		for (int t = 0; t < turns; t++) {
			...
			modeler.observePreState(game.getState());
			game.chosenAction = (t < turns / 4 ? chimp : explorer).getOptimalAction(game.getState(), ACTION_CHOICES, 0, 0);
			modeler.observeAction(game.chosenAction);
			game.oneTurn();
			modeler.observePostState(game.getState());
...
//		WeightPruner.inOutAbsConnWgt(modeler.getModelVTA().getNeuralNetwork());
		...
		for (int numPlanSteps = 1; numPlanSteps <= maxNumPlanSteps; numPlanSteps++) {
			...
			game.setPlayerPos(size-1, size-1);
			game.setOpponentPos(0, 0);
			Planner follower = Planner.createMonteCarloPlanner(modeler, numPlanSteps, numPlanRuns, game.follow, true, 0.1, joints);
			Planner evader = Planner.createMonteCarloPlanner(modeler, numPlanSteps, numPlanRuns, game.evade, true, 0.1, joints);
...
//				game.print(game.chosenAction);
	...
	private final RewardFunction follow = tagReward(true,cols,rows);
	private final RewardFunction evade = tagReward(false,cols,rows);
